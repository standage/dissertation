...
And, maybe even more intriguingly, to what extent is genome organization functionally important?%\volker[*]{Make sure that we address all these nice questions from the last three paragraphs in the applications!}
...
\subsubsection*{Neighbor statistics}
...
\subsection*{Genomic positioning of giLoci}
...
To investigate the extent of gene conservation in the green algae (phylum: Chlorophyta), we collected and processed data for nine chlorophyte species, as well as four land plants as an outgroup.
%(\textit{Auxenochlorella protothecoides}, \textit{Chlamydomonas reinhardtii}, \textit{Chlorella variabilis}, \textit{Coccomyxa subellipsoidea}, \textit{Micromonas pusilla}, \textit{Micromonas sp. RCC299}, \textit{Ostreococcus lucimarinus}, \textit{Ostreococcus tauri}, and \textit{Volvox carteri})
%(\textit{Arabidopsis thaliana}, \textit{Brachypodium distachyon}, \textit{Medicago truncatula}, and \textit{Oryza sativa})
...
%\textit{Intergenic iLoci (iiLoci)} capture and provide convenient access to information about the largest unannotated regions of the genome.
%Furthermore, overlapping or abutting giLoci can be merged with a trivial amount of post-processing, and characteristics of these \textit{merged iLoci (miLoci)}---length, gene count, length of flanking iiLoci---provide insight into the largest and most gene dense regions of the genome.
%Together, miLoci and iiLoci provide an excellent resource for investigating genome organization.
%
%Although no iiLoci are reported for pairs of adjacent genes separated by little or no intergenic space, tracking these cases as \textit{zero-length iiLoci (ziLoci)} can provide additional insight into genome organization.
%For instance, when reporting the length of a giLocus' two flanking iiLoci, ziLoci provide valuable information on the percentage of genes in the genome that are flanked on both sides by intergenic space, those that have intergenic space only on one flank, and those flanked tightly on both sides by other genes.
%Additionally, calculating and plotting aggregate lengths of $N$ adjacent iiLoci provides insight into the spatial distribution of genes when ziLoci are included.
%Investigating multiple small values of $N$ reveals the extent to which genes are clustered in regions of high gene density in the genome.
%
...
%We implemented a procedure to produce random non-overlapping arrangements of giLoci on a genomic sequence.
...
%
%For XX species, we selected six 1Mb regions each from the genome, randomly distributed the giLoci from that region, re-ran the iLocus parsing procedure, and compared length, neighbor statistics, and other characteristics between the actual annotations and the randomly distributed annotations.
...
%
%We confirmed that genes in \textit{Daphnia} are on average shorter than genes in \textit{Drosophila}, despite containing more introns per gene (see \textbf{Figures \ref{Fig:siLocusLengthModOrg}-\ref{Fig:piLocusExonCount}}).\daniel[*]{I will probably end up moving all figures in this paragraph to the supplement. I'll keep them in for now for simplicity's sake.}
...
%The average intron length is indeed shorter (283 bp versus 1,211 bp in \textit{Drosophila}), but the median length is indistinguishable (75 bp versus 72 bp in \textit{Drosophila}), indicating fewer introns in the long tail of the length distribution in \textit{Daphnia} (\textbf{Figure \ref{Fig:piLocusIntronLength}}).
...
%The exon length distribution in both species is much less biased than that of introns, and exons in \textit{Daphnia} (mean: 227 bp, median 154 bp) are approximately half the length of exons in \textit{Drosophila} (mean: 514 bp, median: 286 bp; see \textbf{Figure \ref{Fig:piLocusExonLength}}).
...
\begin{figure}[!bht]
\includegraphics[scale=0.5]{Assets/Graphics/iLoci/ilocus-designations.png}
...
\begin{figure}[!bht]
\includegraphics[scale=0.6]{Assets/Graphics/iLoci/ilocus-demo.pdf}
\caption{Parsing an annotated genome sequence into iLoci.
...
\begin{figure}[h]
\includegraphics[scale=0.45]{Assets/Graphics/iLoci/compactness-theoretical.png}
...
\begin{figure}[h]
\includegraphics[scale=0.45]{Assets/Graphics/iLoci/modorg-compactness.png}
...
\begin{figure}[h]
\includegraphics[scale=0.45]{Assets/Graphics/iLoci/compactness-shuffled.png}
\caption{Genomic compactness as measured on genomic sequences with random giLocus positioning.}
...
\begin{figure}[h]
\includegraphics[scale=0.35]{Assets/Graphics/iLoci/atha-stable.png} \par
\vspace{50px}
\includegraphics[scale=0.35]{Assets/Graphics/iLoci/amel-stable.png}
...
\begin{figure}[h]
\includegraphics[scale=0.35]{Assets/Graphics/iLoci/atha-stable-scatter.png}
\includegraphics[scale=0.35]{Assets/Graphics/iLoci/amel-stable-scatter.png}
...
\begin{figure}[h]
\includegraphics[scale=0.35]{Assets/Graphics/iLoci/algae-bd-counts.png} \par
\vspace{50px}
\includegraphics[scale=0.35]{Assets/Graphics/iLoci/algae-bd-bp.png}
...
\begin{figure}[h]
\includegraphics[scale=0.4]{Assets/Graphics/iLoci/crei-breakdown-scatter.png} \par
